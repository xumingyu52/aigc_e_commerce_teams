...
function getResultVideoUrl(result: VideoGenerationTask['result']): string | null {
  if (!result) {
    return null
  }
  ...
  if (typeof result === 'string') {
    try {
      const parsed = JSON.parse(result) as VideoGenerationResult
        ...
        {hasTasks ? (
          <Table className='w-full'>
            <Table.ScrollContainer>
              <Table.Content aria-label='视频生成历史' className='w-full'>
                <Table.Header>
                  <Table.Column
                ...
                <Table.Body items={tasks}>
                  {(task) => {
                    const videoUrl = getResultVideoUrl(task.result)
                    const statusMeta =
